    ...
    public String aroundAddIncludeTagPointcutScenario(Resource resource) throws Throwable {
        ...
        for (int i = 0; i < lines.size(); i++) {
            if (lines.get(i).contains("@include")) {
                ...
                for (int fu=i; fu<lines.size();fu++) {
                    if (lines.get(fu).contains("@include")) {
                        featureName = getFeatureName(lines.get(fu));
                        scenarioName = getScenName(lines.get(fu));
                        params = getParams(lines.get(fu));
                        lines.set(fu, "");
                        lineToinclude += featureStepConverter(pth + featureName, scenarioName, params);
                    }else{
                            break;
                    }
                ...
                for (int x = i; x < lines.size(); x++) {
                    ...
                    if (lines.get(x).toUpperCase().contains("FEATURE:") && !lines.get(x).toUpperCase().contains("@INCLUDE")) {
                        for (int y = x; y < lines.size(); y++) {
                            if (lines.get(x).toUpperCase().contains("BACKGROUND:") && !marked) {
                                lines.set(i, lines.get(x));
                                lines.set(x, lines.get(y));
                                lines.set(y, lineToinclude);
                                marked = true;
                                lineToinclude= "";
                                break;
                            }
                    ...
                    if ((lines.get(x).toUpperCase().contains("SCENARIO:")||lines.get(x).toUpperCase().contains("OUTLINE:")) && !lines.get(x).toUpperCase().contains("@INCLUDE")) {
                        lines.set(i, lines.get(x));
                        lines.set(x, lineToinclude);
                        lineToinclude = "";
                        break;
                    }
                }
            ...
            logger.error("Escrita:" + lines.get(i));
        }
        logger.error(nwsource);
        ...
        try {
            ...
            while ((sCurrentLine = br.readLine()) != null) {
                if (sCurrentLine.contains(scenarioName)){
                    ...
                    while((sCurrentLine = br.readLine())!=null){
                        if(!sCurrentLine.toUpperCase().contains("SCENARIO:") && !sCurrentLine.toUpperCase().contains("EXAMPLES:") && !sCurrentLine.contains("|")) {
